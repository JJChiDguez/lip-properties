...
def main():
    ...
    if main.count == 1:
        print(f'\n# n:     \t{n}')
        print(f'# Oracle:\tIQFP oracle as in Lemma 3')
    ...
    # Main calculations
    # Decorate function() with the counter() decorator
    @counter
    def function(v, p):  # It computes: (V⁻¹)ᵀ×Q×(V⁻¹)
        return group_action(v.inverse(), p)

    # Decorate oracle_call() with the counter() decorator
    @counter
    def oracle_call(v: matrix):
        # It simulates: given [Vᵀ×(U⁻¹)ᵀ]×Q×[(U⁻¹)×V], returns Uᵀ×((V⁻¹)ᵀ×Q×(V⁻¹))×U
    ...
    print(f'# {group_action.count} group action calls ({function.count} of them concern with input V⁻¹)')
